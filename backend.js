...
app.get("/Chat.json", async (req, res) => {
  const data =  await fetch("https://cap11-data-default-rtdb.europe-west1.firebasedatabase.app/c.json");
  const text = await data.text();     // get raw data
  const json = JSON.parse(text);
  const json2 = Object.values(json)
  console.log(json2)
...
app.get("/ADDMESS", async (req, res) => {
  ...
  console.log("Raw query:", decodeURIComponent(req.query.array))
    var file = await fetch("https://cap11-data-default-rtdb.europe-west1.firebasedatabase.app/c.json")
      ...
      a.push(JSON.parse(req.query.array))
      console.log(a)
      var x = await fetch("https://cap11-data-default-rtdb.europe-west1.firebasedatabase.app/c.json",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})
      console.log(await x.text())
      res.send("trade added")
    });
